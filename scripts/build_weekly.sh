...
if [ "$EXTRA_HOST_ENV" == "true" ]; then
    ...
    sysroot_flags+="--sysroot=$TOOLCHAIN_BASE_PATH/gcc/linux-x86/host/x86_K-linux-glibc2.17-4.8/sysroot "
...
if git rev-parse --verify main >/dev/null 2>&1; then
    ...
    # Exclude the first element for patching
...
# Pass base kernel name to anykernel script
