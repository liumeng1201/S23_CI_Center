...
# --- NEW: Handle both versioning methods ---
...
# --- NEW: Clean up localversion file if created ---
if [ "$VERSION_METHOD" == "file" ]; then
    echo -n > ./localversion
fi
# --- END NEW ---
...
if [[ "$DO_PATCH_LINUX" == "true" ]]; then
    ...
    # The patch_linux script is now expected to be in the AnyKernel repo, which is checked out in the parent dir
    if [ -f "../anykernel_patcher_repo/patch_linux" ]; then
        ...
        echo "--- WARNING: patch_linux script not found in AnyKernel repo! ---"
    fi
fi
